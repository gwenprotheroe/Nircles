...
document.addEventListener('DOMContentLoaded', function () {
            const canvas = document.getElementById('lifeViz');
            ...
            const extinctLegendItems = document.getElementById('extinctLegendItems');
            ...
            const idNumberLegendItems = document.getElementById('idNumberLegendItems');
            const minDateLabel = document.getElementById('minDateLabel');
            const maxDateLabel = document.getElementById('maxDateLabel');
            const minidNumberLabel = document.getElementById('minidNumberLabel');
            const maxidNumberLabel = document.getElementById('maxidNumberLabel');
            ...
            // Define color scales
            const eXtinctColorScale = d3.scaleOrdinal(['#ED6631','#1A845C','#2A73D9','#F3BE0A','#E78F89',
            ...
            const idNumberColorScale = d3.scalePow().exponent(0.1).range(["#2B2A28","#F2EFEC"]); //d3.scaleSequential(d3.interpolateBlues); // Using interpolateRainbow for all hues
            ...
            const dateColorScale = d3.scaleLinear().range(["#000000","#F2EFEC"]); // Shades of black and white for folder depth
  ...
  "last_modified_iso": "2025-08-11T00:39:34.205280"
            ...
            function drawVisualization() {
                ...
                applyCurrentZoom();
                folderSummary.textContent = currentDataNodes[0].data.name + " contains " + currentDataNodes.length + " items.";
...
/////////////////////// Draw the Files ///////////////////////////////////
	...
	//////////////////////////////////////////////////////////////
            function setColorDomains() {
                ...
                const eXtinct = Array.from(new Set(currentDataNodes.filter(d => !d.children && d.data.type).map(d => d.data.type)));
                eXtinctColorScale.domain(eXtinct);

                const allDates = currentDataNodes.filter(d => d.data.last_modified_unix).map(d => d.data.last_modified_unix);
                const minDate = d3.min(allDates);
                const maxDate = d3.max(allDates);
                dateColorScale.domain([minDate, maxDate]);

                const allidNumbers = currentDataNodes.filter(d => !d.children).map(d => d.value);
                const minidNumber = d3.min(allidNumbers) || 0;
                const maxidNumber = d3.max(allidNumbers) || 1; // Avoid division by zero
                idNumberColorScale.domain([minidNumber, maxidNumber]);
                ...
                fileDepthColorScale.domain([0, 15]);
                ...
                //minidNumberLabel.textContent = minidNumber ? formatBytes(minidNumber) : 'Smallest';
                //maxidNumberLabel.textContent = maxidNumber ? formatBytes(maxidNumber) : 'Largest';

                updateColorLegend(eXtinct);
            }
            ...
            function getNodeColor(d) {
                ...
                if (colorMode === "depth") {
                    ...
                    return d.data.last_modified_unix ? dateColorScale(d.data.last_modified_unix) : "#cccccc"; // Default grey if no date
                } else if (colorMode === "idNumber") {
                    return idNumberColorScale(d.value); //Files also show size
                } else {
                    if (!d.children) { // It's a directory (folder)
                    return eXtinctColorScale(d.data.type);
                }
                return idNumberColorScale(d.value); // Directories always show gradient
                }
                return "#E2DDDB"; // Fallback default color
            }
	...
	////////////////////////////////////////////////////////////// 
                ...
                if (searchTerm.length > 1){
                    ...
                    // Add items for search results
                            searchResults.forEach(path => {
                                ...
                                resultItem.innerHTML = `${path.data.name}`;
            ...
            // Create the legend containing all extinct species
            function updateColorLegend(eXtinct) {
                const colorMode = colorBySelect.value;

                // Hide all legends initially
                extinctLegendItems.classList.add('hidden');
                dateLegendItems.classList.add('hidden');
                depthLegendItems.classList.add('hidden');
                idNumberLegendItems.classList.add('hidden');

                if (colorMode === "type") {
                    extinctLegendItems.classList.remove('hidden');
                    extinctLegendItems.innerHTML = ''; // Clear previous legend items
                    ...
                    folderLegendItem.innerHTML = `
                        ...
                        <span>Folder</span>
                    `;
                    extinctLegendItems.appendChild(folderLegendItem);

                    // Add legend items for extinct species
                    eXtinct.forEach(type => {
                        const legendItem = document.createElement('div');
                        legendItem.className = 'flex items-center space-x-2';
                        legendItem.innerHTML = `
                        <span class="w-4 h-4 rounded-full" style="background-color: ${eXtinctColorScale(type)};"></span>
                            <span>.${type}</span>
                        `;
                        extinctLegendItems.appendChild(legendItem);
                    });
                } else if (colorMode === "date") {
                    ...
                    idNumberLegendItems.classList.remove('hidden');
                }
            }
