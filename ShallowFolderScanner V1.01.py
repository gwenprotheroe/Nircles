...
def create_folder_structure_json_bfs(path, current_depth=0):
    """
    ...
    modification times. It will not scan beyond the specified depth.

    Args:
        path (str): The root path of the folder to scan.
        current_depth (int): The current depth of the scan, starting from 0.
    ...
    queue = deque()
    # The queue now stores tuples of (path, parent_children_list, depth)
            ...
            with os.scandir(current_dir_path) as entries:
                for entry in entries:
                    ...
                    if entry.is_dir():
                        ...
                        # Only add to the queue if the current depth is less than 2
                        if current_level < 1:
...
def scan_folder_concurrently(root_path):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count() * 2) as executor:
        # Pass the initial depth of 1 for the top-level subdirectories
        future_to_dir = {executor.submit(create_folder_structure_json_bfs, sub_dir, 1): sub_dir for sub_dir in sub_dirs_to_scan}
